...
<template>
    <i-article>
        <article>
            <h1>定制主题</h1>
            <p>Vued 默认提供了一套 UI 主题，并且可以在一定程度上定制新主题，以满足业务和品牌上的多样化视觉需求。</p>
            <p>Vued 的样式是基于 <a href="http://lesscss.org/" target="_blank">Less</a> 进行开发的，默认以前缀 <code>.ivu-</code> 作为命名空间，并且定义了一套样式变量，定制主题，就是编辑这个变量列表。</p>
            ...
            <p>
                完整的变量列表可以查看 <a href="https://github.com/guanjingkai/vued/blob/master/src/styles/custom.less" target="_blank">默认样式变量</a>。
            </p>
            ...
            <p>如果没有使用 webpack，可以用我们提供的工具 <a href="https://github.com/guanjingkai/vued-theme" target="_blank">vued-theme</a> 来编译。</p>
            <p>首先需要安装<strong>主题生成工具</strong>，从 npm 全局或在项目中局部安装，以全局安装为例：</p>
            <i-code lang="auto" bg>{{ code.theme.install }}</i-code>
            <p>然后在业务工程里新建一个目录，用来存放主题文件，使用下面的命令初始化主题，这时会从 Vued 仓库拉取最新的样式文件：</p>
            ...
            <p>最终会在指定的目录下编译为 <code>vued.css</code> 的文件，只需在入口处引用它就可以了，比如在 main.js 文件：</p>
            <i-code lang="js" bg>{{ code.theme.import }}</i-code>
            <blockquote>注意：这种方法是从 GitHub 拉取最新的样式变量，并不是你当前的 Vued 版本，另外在更新 Vued 后，应该重新拉取。</blockquote>
        </article>
    </i-article>
</template>
<script>
